...
if 当前为「盘中交易」时段：
  ...
  → 静默，回复：
    "📌 当前为午间休市（11:30-13:00），异动监控已暂停。
     将于 13:00 下午开盘后自动恢复推送。如需查看，请发送「刷新」。"

elif 当前为「盘后」时段（15:00 后的交易日）：
  → 静默，回复：
    "📌 今日 A 股已收盘（15:00），异动监控已停止实时推送。
     如需查看今日收盘综述或持仓复盘，请发送「今日复盘」。"

elif 当前为「周末 / 节假日」：
  → 静默，回复：
    "📌 当前为非交易日（周末/节假日），A 股休市，异动监控不推送。
     如需查看本周复盘或持仓组合分析，请发送「本周复盘」。"
```
...
【⏰ 交易时段门控 — 自动推送异动报告前必须执行】
  ...
  ├─ 午间休市（11:30-13:00）→ ❌ 静默，提示午休中
  ├─ 盘后（15:00 后）→ ❌ 静默，提示切换复盘模式
  └─ 非交易日（周末/节假日）→ ❌ 静默，提示切换周末复盘
  │
