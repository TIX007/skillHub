...
version: 1.2.0
description: A股智能持仓监控与投研助手。支持截图识别持仓个股、全时段行情异动监控、实时新闻/公告/舆情抓取、基本面+技术面分析、量化风险评估与实操投资建议。内置自进化记忆系统（记忆块索引+睡眠反思总结）、Python精准推送控制（交易日历+防重复+漏推检测）与T操作推荐系统（250天低点+资金流向双维度打分→正T/反T/观望百分比）。触发词：持仓监控、持仓分析、异动监控、股票分析、持仓截图、添加监控、持仓复盘、调入调出、看盘、盯盘、选股、A股分析、股票新闻、公告解读、龙虎榜。
agent_created: true
tags: A股,股票监控,识图持仓,舆情新闻,异动提醒,投资分析,持仓管理,自进化,记忆系统,精准推送,T操作,正T,反T
---
...
| T操作推荐 | westock-data（历史K线+资金流向）       | 250天低点位置×资金流向双维度打分→正T/反T/观望百分比+仓位执行规则 |
...
## 模块 8：T 操作推荐系统（250天低点 × 资金流向）

### 设计理念

通过**两个客观维度**量化 T 操作方向：
1. **离 250 天历史低点的远近**（位置维度：越靠近底部越宜正 T）
2. **近 3-5 日资金流向趋势**（动量维度：资金持续进场则正 T，持续出场则反 T）

两个维度各贡献 0-50 分，合成 0-100 总分，再映射成**正 T / 反 T / 观望百分比**。

> 核心逻辑：低位 + 资金进 → 正 T（低吸）；高位 + 资金出 → 反 T（高卖）；中间地带 → 观望为主，轻仓尝试。

---

### 8.1 维度 A：相对 250 天低点位置（0–50 分）

**必须数据**：250 天最低价 L、最高价 H、当前价 P

```
相对低位 = (P − L) / (H − L)   值越小 → 越靠近底部
```

使用 `westock-data kline [code] --period day --limit 250` 获取 250 日 K 线，取 High/Low 极值。
...
### 8.2 维度 B：资金流向趋势（0–50 分）

**必须数据**：近 3–5 日主力净流入/流出金额（`westock-data asfund [code]`）

| 资金状态                        | 量价配合    | B 得分 |
| --------------------------- | ------- | ----- |
| 连续 3 日净流入 + 股价不跌/微涨         | 强多头     | 50 分  |
| 2 日流入、1 日流出，整体净流入           | 偏多      | 40 分  |
| 震荡、资金小幅波动、无明显方向             | 中性      | 25 分  |
| 2 日流出、1 日流入，整体净流出           | 偏空      | 10 分  |
| 连续 3 日净流出 + 股价下跌            | 强空头     | 0 分   |
...
| 总分区间      | 市场含义        | 正 T % | 反 T % | 观望 % | 操作逻辑                   |
| --------- | ----------- | ----- | ----- | ----- | ---------------------- |
| **S ≥ 80**    | 低位 + 强资金流入  | **70%** | 10%   | 20%   | 底部 + 资金进场 → 回调低吸，少做反 T |
| **65 ≤ S ≤ 79** | 低位 + 偏多资金  | **55%** | 25%   | 20%   | 底部区震荡偏强 → 正 T 为主，少量反 T |
| **50 ≤ S ≤ 64** | 中位 + 中性资金  | 35%   | 35%   | **30%** | 无明显方向 → 轻仓两边做，多观望     |
| **35 ≤ S ≤ 49** | 中高位 + 偏空资金 | 25%   | **55%** | 20%   | 高位 + 资金流出 → 冲高就卖，少正 T  |
| **S < 35**    | 高位 + 强资金流出  | 10%   | **70%** | 20%   | 250天高位 + 资金出逃 → 只做反 T   |
...
| 仓位类型 | 比例  | 用途                |
| ---- | --- | ----------------- |
| 底仓   | 30% | 只持有，不参与做 T，保留盈利弹性 |
| 机动仓  | 70% | 专用于做 T，单次只用 1/3–1/2 |

> ⚠️ 单次做 T 只用机动仓的 **1/3–1/2**，绝对禁止满仓做 T。
...
- 黄白分时线距离 ≤ 3 格、振幅 < 1%
- 单边大涨/大跌、放量不回头（趋势过强，做 T 风险极大）
- 当日已触及涨跌停（竞价/尾盘集合竞价除外）
- 重大利空/利好公告刚发布，价格尚未稳定
...
当用户发起「T操作分析」或「正反T建议」时，执行以下步骤：

```
Step 1：获取 250 日 K 线（并行）
  westock-data kline [code] --period day --limit 250
  → 提取：L（250日最低价）、H（250日最高价）、P（当前价）

Step 2：获取近 5 日资金流向（并行）
  westock-data asfund [code]
  → 提取：近 3-5 日主力净流入/流出方向
...
| 维度 | 得分 | 依据 |
|------|------|------|
| A：位置分（0-50） | X 分 | 相对低位 X%，处于[低位/中位/高位]区间 |
| B：资金分（0-50） | X 分 | 近3日[连续流入/偏多/中性/偏空/连续流出] |
| **总分 S** | **X 分** | A + B |
...
### 🎯 执行指引
- **仓位**：底仓 30% 不动，机动仓 70% 中用 1/3–1/2 做 T
- **[正/反 T] 触发**：[日内具体信号，如"急跌≥3%缩量企稳买入，反弹3%卖出"]
- **止损线**：[低于/高于 X 元止损]

### ⚠️ 风险提示
- [当前市场风险点]
- [做 T 特有风险：如振幅不足/趋势过强等]
...
当用户提供数据但希望 AI 快速计算时，可直接套用以下速算表：

```
【T 操作速算表】

250天最低 L = ____  最高 H = ____  现价 P = ____

相对低位 = (P - L) / (H - L) = ____  → A = ____ 分

近 3-5 日资金：
  □ 连续 3 日流入 + 股价不跌  → B = 50 分
  □ 2 日流入 1 日流出，整体净流入  → B = 40 分
  □ 震荡、无明显方向  → B = 25 分
  □ 2 日流出 1 日流入，整体净流出  → B = 10 分
  □ 连续 3 日流出 + 股价下跌  → B = 0 分

总分 S = A + B = ____

结论：正 T ___%  反 T ___%  观望 ___%
```
...
| 用户输入                           | 响应行为                                  |
| ------------------------------ | ------------------------------------- |
| `T操作分析 600519`               | 自动获取250日K线+资金数据 → 完整输出8.6格式报告      |
| `正反T建议 [代码]`                  | 同上                                    |
| `做T分析 [代码]`                   | 同上                                    |
| `[代码] 现在适合正T还是反T？`           | 同上                                    |
| 用户提供 L/H/P + 资金方向            | 直接计算得分，不需调用数据接口                     |
| `批量T操作分析 600519 000858 300750` | 并行获取三只股票数据 → 输出对比表格，标注各自推荐方向        |
